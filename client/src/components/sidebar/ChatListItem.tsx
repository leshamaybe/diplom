...
const ChatListItem = ({ data, ...props }: { data: IUser }) => {
    const params = useParams();
    const { id } = params;

    const isActive = Number(id) === data?.profile.id;

    return (
        <Link {...props} href={`/dashboard/${data?.profile.id}`}>
            <div
                className={cn(
            ...
            >
                <Avatar className="absolute" username={data?.profile?.name} />
